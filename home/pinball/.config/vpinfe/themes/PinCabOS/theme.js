...
// PINCABOS_POWERED_IP_JS_V1

let pincabIpRefreshTimer = null;


function isPinCabOSUsableIPv4(value) {
...
function extractPinCabOSCabIPv4(remoteInfo) {
...
async function refreshPinCabOSCabIp() {
    ...
    try {
        ...
        console.warn(
            "PinCabOS: unable to detect cabinet IP",
            error
        );
...
function setupPinCabOSCabIp() {

    if (windowName !== "table") {
        return;
    }

    refreshPinCabOSCabIp();

    if (pincabIpRefreshTimer) {

        window.clearInterval(
            pincabIpRefreshTimer
        );
    }

    /*
     * Rafraichissement toutes les 30 secondes.
     * Si DHCP/Wi-Fi/Ethernet change, l'adresse
     * affichée se corrige sans redémarrer VPinFE.
     */
    pincabIpRefreshTimer =
        window.setInterval(
            refreshPinCabOSCabIp,
...
// PINCABOS_ATTRACT_VISUAL_JS_V1
function setPinCabOSAttractVisualState(active, broadcast = false) {
    const enabled = Boolean(active);

    document.body.classList.toggle(
        "pincabos-attract-mode",
        enabled
    );

    if (broadcast && windowName === "table") {
        vpin.sendMessageToAllWindows({
            type: "PinCabOSAttractVisualState",
            active: enabled
        });
    }
}
...
function runAttractAdvance() {
    ...
    /*
     * Sélection uniformément aléatoire parmi toutes
     * les tables SAUF celle actuellement affichée.
     *
     * Avec N tables, on choisit d'abord entre
     * 0 et N-2, puis on saute l'index courant.
     *
     * Résultat :
     * - aucune progression alphabétique
     * - toutes les autres tables ont la même chance
     * - aucune répétition immédiate
     */
    let nextIndex = Math.floor(
        Math.random() * (tableCount - 1)
    );
...
async function receiveEvent(message) {
    ...
    if (message.type === "PinCabOSAttractVisualState") {
        setPinCabOSAttractVisualState(
