...
updates). L'action reste synchrone : statut, boutons et journal sont retournes
sans ajouter un second moteur de mise a jour.
"""
...
function rendreEtat(s) {
  if (s.error) { etat.innerHTML = '<span style="color:#e66">Erreur : ' + esc(s.error) + '</span>'; return; }
  const inst = s.installed || 'aucune';
  const cible = s.target || '?';
  const ajour = s.up_to_date;
  etat.innerHTML =
    '<div>Version installee : <b>' + esc(inst) + '</b></div>' +
    '<div>Version cible : <b>' + esc(cible) + '</b></div>' +
