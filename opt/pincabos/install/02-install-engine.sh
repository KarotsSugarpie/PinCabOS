...
write_global_paths_config() {
  ...
  # /opt/pincabos/tools/pincabos_paths.py ; ce fichier ne porte que les
  # surcharges (aucune a l'installation).
  cat >/opt/pincabos/config/pincabos-paths.json <<EOF_PATHS
{
  "schema": "pincabos.paths/2",
  "comment": "Surcharges des chemins PinCabOS. Defauts dans /opt/pincabos/tools/pincabos_paths.py ; n'ecrire ici que ce qui differe.",
  "user": "pinball",
  "paths": {
    "tables": "$TABLES_DIR"
  }
...
ensure_vpx_ini() {
  pco_step "09" "Ensure VPX cabinet defaults without destroying existing file"
  ...
  python3 - "$VPX_INI" <<'PY_VPX_DEFAULTS'
import configparser
import sys
...
check = configparser.ConfigParser(interpolation=None, strict=False)
with tmp.open(encoding="utf-8") as handle:
    check.read_file(handle)

for section, values in defaults.items():
    for key in values:
        if not check.has_option(section, key):
            raise SystemExit(f"Missing VPX default: [{section}] {key}")

tmp.replace(path)
PY_VPX_DEFAULTS

  chown pinball:pinball "$VPX_INI" 2>/dev/null || true
  pco_go "VPX cabinet defaults ensured: $VPX_INI"
}

ensure_vpinfe_ini() {
  pco_step "10" "Ensure VPinFE cabinet defaults"
  ...
  python3 - "$VPINFE_INI" <<'PY_VPINFE_DEFAULTS'
import configparser
import sys
from pathlib import Path

path = Path(sys.argv[1])
template = '[Paths]\nvpxbinpath = /opt/pincabos/bin/vpx.sh\ntablespath = /home/pinball/Tables\nmediapath = /opt/pincabos/media\n\n[Server]\nhost = 127.0.0.1\nport = 8000\napiport = 8001\n\n[PinCabOS]\nenabledof = true\nvpxbinpath = /opt/pincabos/bin/vpx.sh\nvpxinipath = /home/pinball/.vpinball/VPinballX.ini\nvpxdir = /opt/pincabos/apps/vpinball\nvpinfedir = /opt/pincabos/apps/frontend/vpinfe/current\nromsdir = /opt/pincabos/apps/vpinball/PinMAME/roms\n\n[Settings]\ntablerootdir = /home/pinball/Tables\nvpxbinpath = /opt/pincabos/bin/vpx.sh\nvpxinipath = /home/pinball/.vpinball/VPinballX.ini\nthemeassetsport = 8001\nmanageruiport = 8000\nvpxlaunchenv =\ntheme = Trinidad\nsplashscreen = false\nmuteaudio = true\n\n[Displays]\nbgscreenid = 1\ndmdscreenid = 2\nbgwindowoverride =\ndmdwindowoverride =\ntablescreenid = 0\ntableorientation = landscape\ntablerotation = 0\ncabmode = true\ntablewindowoverride =\n\n[Input]\nkeyleft = ArrowLeft,ShiftLeft\nkeyright = ArrowRight,ShiftRight\nkeyup = ArrowUp\nkeydown = ArrowDown\nkeypageup = PageUp\nkeypagedown = PageDown\nkeyselect = Enter\nkeymenu = m\nkeyback = b\nkeytutorial = t\nkeyexit = Escape,q\nkeycollectionmenu = c\n\n[Media]\ntabletype = table\ntableresolution = 4k\ntablevideoresolution = 1k\ntablemediapriority = video\nbgmediapriority = video\ndmdmediapriority = video\nrealdmdmediapriority = color\n'

base = configparser.ConfigParser(interpolation=None, strict=False)
base.optionxform = str
base.read_string(template)
...
forced = (
    ("Paths", "vpxbinpath", "/opt/pincabos/bin/vpx.sh"),
    ("Paths", "tablespath", "/home/pinball/Tables"),
    ("Paths", "mediapath", "/opt/pincabos/media"),
    ("Settings", "tablerootdir", "/home/pinball/Tables"),
    ("Settings", "vpxbinpath", "/opt/pincabos/bin/vpx.sh"),
    ("Settings", "vpxinipath", "/home/pinball/.vpinball/VPinballX.ini"),
    ("PinCabOS", "vpxbinpath", "/opt/pincabos/bin/vpx.sh"),
    ("PinCabOS", "vpxinipath", "/home/pinball/.vpinball/VPinballX.ini"),
    ("PinCabOS", "vpxdir", "/opt/pincabos/apps/vpinball"),
    ("PinCabOS", "vpinfedir", "/opt/pincabos/apps/frontend/vpinfe/current"),
    ("PinCabOS", "romsdir", "/opt/pincabos/apps/vpinball/PinMAME/roms"),
)

for section, key, value in forced:
    if not cfg.has_section(section):
        cfg.add_section(section)
    cfg.set(section, key, value)

cabmode = cfg.get("Displays", "cabmode", fallback="").strip().lower()
if cabmode in ("", "0", "false", "no", "off"):
    cfg.set("Displays", "cabmode", "true")
...
for section, key in (
    ("Settings", "theme"),
    ("Displays", "tablescreenid"),
    ("Displays", "bgscreenid"),
    ("Displays", "dmdscreenid"),
    ("Displays", "cabmode"),
):
    if not check.has_option(section, key):
        raise SystemExit(f"Missing VPinFE default: [{section}] {key}")

tmp.replace(path)
PY_VPINFE_DEFAULTS

  chown pinball:pinball "$VPINFE_INI" 2>/dev/null || true
  pco_go "VPinFE cabinet defaults ensured: $VPINFE_INI"
}
...
USER_CFG="$HOME/.config/vpinfe/vpinfe.ini"

mkdir -p "$(dirname "$USER_CFG")"

python3 - "$USER_CFG" <<'PY_VPINFE_USER_DEFAULTS' >>"$LOG" 2>&1
import configparser
import os
import sys
from pathlib import Path
...
print(f"VPinFE user config guard: changed={changed} path={path}")
PY_VPINFE_USER_DEFAULTS

echo "VPINFE_USER_CFG_GUARD=$USER_CFG" >>"$LOG"
